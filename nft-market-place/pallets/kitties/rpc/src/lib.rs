...
pub trait TestRpcFunction<BlockHash> {
	#[rpc(name = "my_get_kitties_cnt")]
	fn rpc_get_kitties_cnt(&self, at: Option<BlockHash>) -> Result<u64>;
}
	...
	C::Api: TestRuntimeApi<Block>
{
	...
	fn rpc_get_kitties_cnt(
		&self,
		at: Option<<Block as BlockT>::Hash>,
	) -> Result<u64> {
		let api = self.client.runtime_api();
		let at: BlockId<Block> = BlockId::hash(at.unwrap_or_else(||
			// If the block hash is not supplied assume the best block.
			self.client.info().best_hash));

			let result_api = api.api_get_kitties_cnt(&at);
